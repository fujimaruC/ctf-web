...
3. For Firebase development, copy `.env.example` to `.env.local`, set `VITE_DATA_MODE=firebase`, and supply staging configuration. Use the existing Firestore region for both `VITE_FIREBASE_REGION` and the functions `FUNCTIONS_REGION` parameter/environment. Never use production credentials for tests.
...
- Deploy functions and rules only to staging after emulator tests pass. Inspect required indexes in staging; emulators do not enforce all production index requirements.
...
- Reconciliation requires maintenance, a reviewed digest, and a backup. It writes checkpointed batches. An interrupted repair keeps maintenance enabled; after the 10-minute lease expires, preview and apply again. Do not manually reopen during an incomplete repair.
...
3. Deploy the reviewed rules, indexes, functions, and React build. Confirm the region matches Firestore. Keep maintenance enabled.
...
6. Reopen only after review. Watch callable error rates, permission denials, duplicate-award checks, Firestore reads, latency, and billing. Do not log submitted flags or personal profile data.
